...
async function recibirEvento() {
    const connection = await amqp.connect('amqp://paolis:paolis@54.165.239.162');
    const channel = await connection.createChannel();

    const exchange = 'ColaPao'; //nombre del exchange al que apunta

    await channel.assertExchange(exchange, 'direct', { durable: true });

    const queueName = 'colaHexa'; //busca la cola a la que apunta el exchange
    const queue = await channel.assertQueue(queueName, { exclusive: false });
    await channel.bindQueue(queue.queue, exchange, '123');
